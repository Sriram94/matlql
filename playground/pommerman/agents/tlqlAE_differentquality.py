...
from RL_brain_twolevelqlAE import tlqlNetworkAE
...
class twolevelqlAgentAE_differentquality(BaseAgent):
    def __init__(self, nf, sess, *args, **kwargs):
        super(twolevelqlAgentAE_differentquality, self).__init__(*args, **kwargs)
        self._recently_visited_positions = []
        self._recently_visited_length = 6
        self._prev_direction = None
        self.RL = tlqlNetworkAE(sess, 6, nf, 4,
                      learning_rate=0.01,
                      reward_decay=0.9,
                      e_greedy=0.9,
    ...
    def act(self, obs, action_space):        
        ...
        if self.execution == True: 
            ...
            if (np.random.uniform() <= self.eps):
                s =  self.RL.choose_advisor(obs2, self.execution)
            else: 
                s = 4
