...
def main():
    ...
    sess = tf.Session()
    agent_list = [
        agents.matwolevelqlAgent_insufficient_differentqualityoppmod(201, sess),
    ...
    with open('matlqloppmodvsdqn.csv', 'w+') as myfile:
    ...
    for i_episode in range(50000):
        ...
        while not done:
            ...
            agent_list[0].set(state[0], actions[1])
            opp_action = agent_list[0].return_opp_action()
            actions_new = env.act(state_new)    
            agent_list[0].store(state[0], actions[0], actions[1], reward[0], state_new[0], opp_action)
            agent_list[1].store(state[1], actions[1], reward[1], state_new[1])
            agent_list[0].learnoppaction(state[0], actions_new[1], actions[1])
            state = state_new
            actions = actions_new
        ...
        with open('matlqlvsdqnoppmod.csv', 'a') as myfile:
    ...
    #    with open('matlqlvsdqnoppmod.csv', 'a') as myfile:
    #        myfile.write('{0},{1},{2}\n'.format(i_episode, cumulative_rewards[0], cumulative_rewards[1]))
    #    with open('expertfrequencymatlqloppmod.csv', 'a') as myfile:
    #        myfile.write('{0},{1},{2},{3},{4}\n'.format(i_episode, frequency[0], frequency[1], frequency[2], frequency[3]))
    #    print('Episode {} finished'.format(i_episode))
    #env.close()
