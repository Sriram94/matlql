...
def run_mpe(parallel_env):
    step = 0
    with open('pettingzoompematlaclearningadvisor.csv', 'w+') as myfile:
    ...
    while num_episode < 12000:
            ...
            for agent in parallel_env.agents:
                ...
                if "adversary" in agent:
                    ...
                    if old_action_dict:
                        ...
                        RL_adversary.store_transition(old_observation_dict[agent], old_action_dict[agent], old_reward_dict[agent], observation[agent])
                       
                        RL2_adversary.store_transition(old_observation_dict[agent], old_action_dict[agent], old_reward_dict[agent], observation[agent])
                        RL3_adversary.store_transition(old_observation_dict[agent], old_action_dict[agent], old_reward_dict[agent], observation[agent])
                        RL4_adversary.store_transition(old_observation_dict[agent], old_action_dict[agent], old_reward_dict[agent], observation[agent])
        ...
        with open('pettingzoompematlaclearningadvisor.csv', 'a') as myfile:
