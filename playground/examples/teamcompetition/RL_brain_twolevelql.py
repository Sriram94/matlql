...
class tlqlNetwork:
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('tlql_Value', reuse = tf.AUTO_REUSE):
            self.name_scope = tf.get_variable_scope().name 
            with tf.variable_scope('tlql_eval_net_1', reuse = tf.AUTO_REUSE):
                c_names, n_l1, w_initializer, b_initializer = \
                    ['tlql_eval_net_params', tf.GraphKeys.GLOBAL_VARIABLES], 50, \
                    tf.random_normal_initializer(0., 0.3), tf.constant_initializer(0.1)  

                with tf.variable_scope('tlql_l_1', reuse = tf.AUTO_REUSE):
                ...
                with tf.variable_scope('tlql_lh1', reuse = tf.AUTO_REUSE):
                ...
                with tf.variable_scope('tlql_l_2', reuse = tf.AUTO_REUSE):
            ...
            with tf.variable_scope('tlql_loss', reuse = tf.AUTO_REUSE):
                self.loss = tf.reduce_mean(tf.squared_difference(self.q_target, self.q_eval))
            with tf.variable_scope('tlql_train', reuse = tf.AUTO_REUSE):
                self._train_op = tf.train.RMSPropOptimizer(self.lr).minimize(self.loss)

            self.s_ = tf.placeholder(tf.float32, [None, self.n_features], name='tlql_s1')    
            with tf.variable_scope('tlql_target_net_1', reuse = tf.AUTO_REUSE):
    ...
    def _build_net2(self):
        ...
        with tf.variable_scope('tlql_Advisor', reuse = tf.AUTO_REUSE):
            self.name_scope2 = tf.get_variable_scope().name 
            with tf.variable_scope('tlql_advisor_net_1', reuse = tf.AUTO_REUSE):
                c_names, n_l1, w_initializer, b_initializer = \
                    ['tlql_advisor_net_params', tf.GraphKeys.GLOBAL_VARIABLES], 50, \
                    tf.random_normal_initializer(0., 0.3), tf.constant_initializer(0.1)  

                with tf.variable_scope('tlql_l', reuse = tf.AUTO_REUSE):
                ...
                with tf.variable_scope('tlql_l2', reuse = tf.AUTO_REUSE):
            ...
            with tf.variable_scope('tlql_loss2', reuse = tf.AUTO_REUSE):
                self.loss2 = tf.reduce_mean(tf.squared_difference(self.q_target2, self.q_advisor))
            with tf.variable_scope('tlql_train2', reuse = tf.AUTO_REUSE):
