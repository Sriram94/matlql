...
def main():
    ...
    agent_list = [
        agents.matwolevelqlteamAgent(372, sess),
    ...
    with open('matlqlvsdqn.csv', 'w+') as myfile:
        myfile.write('{0},{1},{2},{3},{4}\n'.format("Episode", "Reward1(MATLQL)", "Reward2(DQN)", "Reward1(MATLQL)", "Reward2(DQN)"))
    ...
    for i_episode in range(50000):
        ...
        while not done:
            ...
            agent_list[0].store(state[0], actions[0], action_list, reward[0], state_new[0], action_list_new)
            agent_list[2].store(state[2], actions[2], action_list2, reward[2], state_new[2], action_list_new2)
