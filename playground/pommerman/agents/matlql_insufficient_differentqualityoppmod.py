...
from RL_brain_matwolevelqloppmodel import matlqlNetwork
...
class matwolevelqlAgent_insufficient_differentqualityoppmod(BaseAgent):
    def __init__(self, nf, sess, *args, **kwargs):
        super(matwolevelqlAgent_insufficient_differentqualityoppmod, self).__init__(*args, **kwargs)
        self._recently_visited_positions = []
        self._recently_visited_length = 6
        self._prev_direction = None
        self.RL = matlqlNetwork(sess, 6, nf, 4,
                      learning_rate=0.01,
                      reward_decay=0.9,
                      e_greedy=0.9,
    ...
    def act(self, obs, action_space):
        ...
        obs2 = featurize(obs)
        obs2_str = np.array2string(obs2, precision=2, separator=',')
        if obs2_str in self.state_freq_dict:
            self.state_freq_dict[obs2_str] = self.state_freq_dict[obs2_str] + 1
        else:
            self.state_freq_dict[obs2_str] = 1
        ...
        if (np.random.uniform() <= self.eps):
            ...
            list_frequency = self.state_freq_dict[obs2_str]
            list_frequency = 1/list_frequency
            s =  self.RL.choose_advisor(obs2, self.a2, list_actions, list_frequency, self.execution)
            if s == 0:
            ...
            a = self.RL.choose_action(obs2, self.a2, self.execution)
            self.advisor = 4
            return a
    ...
    def set(self, obs, a2): 
        obs = featurize(obs)
        opp_action = self.RL.get_opp_action(obs, a2)
        self.a2 = opp_action 

    def return_opp_action(self): 
        return self.a2

    def learnoppaction(self, obs, target_action, previous_action): 
        obs = featurize(obs)
        self.RL.learn_opp_action(obs, target_action, previous_action)
    
    def store(self, obs, act, a2, reward, obs_, a2_):
        obs = featurize(obs)
        obs_ = featurize(obs_)
        self.RL.store_transition(obs, act, a2, a2_, reward, obs_, self.advisor)
    ...
    def save_model_advisor(self, s):
        self.RL.save_model_advisor(s)


    def restore_model_advisor(self, s):
        self.RL.restore_model_advisor(s) 
