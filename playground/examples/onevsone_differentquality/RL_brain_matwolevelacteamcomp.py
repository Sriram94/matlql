...
class Actor(object):
    def __init__(self, sess, n_features, n_actions, lr=0.001):
        ...
        with tf.variable_scope('matlactorvalue', reuse = tf.AUTO_REUSE):
            self.name_scope = tf.get_variable_scope().name

            with tf.variable_scope('matlacActor', reuse = tf.AUTO_REUSE):
                l1 = tf.layers.dense(
                    inputs=self.s,
                    units=50,    
            ...
            with tf.variable_scope('matlacexp_v', reuse = tf.AUTO_REUSE):
                log_prob = tf.log(self.acts_prob[0, self.a])
                self.exp_v = tf.reduce_mean(log_prob * self.advantage)

            with tf.variable_scope('matlactrain', reuse = tf.AUTO_REUSE):
...
class Actor2(object):
    def __init__(self, sess, n_features, n_advisors, lr=0.001):
        ...
        with tf.variable_scope('matlactorvalue2', reuse = tf.AUTO_REUSE):
            self.name_scope = tf.get_variable_scope().name

            with tf.variable_scope('matlacActor2', reuse = tf.AUTO_REUSE):
                l1 = tf.layers.dense(
                    inputs=self.s,
                    units=50,    
                    activation=tf.nn.relu,
            ...
            with tf.variable_scope('matlacexp_v2', reuse = tf.AUTO_REUSE):
                log_prob = tf.log(self.acts_prob[0, self.e])
                self.exp_v = tf.reduce_mean(log_prob * self.advantage)

            with tf.variable_scope('matlactrain2', reuse = tf.AUTO_REUSE):
                self.train_op = tf.train.AdamOptimizer(lr).minimize(-self.exp_v)  
...
class Critic(object):
    def __init__(self, sess, n_features, n_actions, lr=0.01):
        ...
        self.s = tf.placeholder(tf.float32, [1, n_features+3], "matlacstate")
        self.a = tf.placeholder(tf.int32, None, "matlacact")
        ...
        with tf.variable_scope('matlaccriticvalue', reuse = tf.AUTO_REUSE):
            self.name_scope = tf.get_variable_scope().name

            with tf.variable_scope('matlacCritic', reuse = tf.AUTO_REUSE):
                l1 = tf.layers.dense(
                    inputs=self.s,
                    units=50,  
                    activation=tf.nn.relu,  
            ...
            with tf.variable_scope('matlacsquared_TD_error', reuse = tf.AUTO_REUSE):
                self.td_error = self.r + GAMMA * self.q_ - self.q[0,self.a]
                self.loss = tf.square(self.td_error)    
            with tf.variable_scope('matlaccritictrain', reuse = tf.AUTO_REUSE):
...
class Critic2(object):
    def __init__(self, sess, n_features, n_advisors, lr=0.01):
        ...
        with tf.variable_scope('matlaccriticvalue2', reuse = tf.AUTO_REUSE):
            self.name_scope = tf.get_variable_scope().name


            with tf.variable_scope('matlacCritic2', reuse = tf.AUTO_REUSE):
                l1 = tf.layers.dense(
                    inputs=self.s,
                    units=50,  
                    activation=tf.nn.relu,  
            ...
            with tf.variable_scope('matlacsquared_TD_error2', reuse = tf.AUTO_REUSE):
                self.td_error = self.r + GAMMA * self.q_ - self.q[0,self.ex]
                self.loss = tf.square(self.td_error)    
            with tf.variable_scope('matlaccritictrain2', reuse = tf.AUTO_REUSE):
                self.train_op = tf.train.AdamOptimizer(lr).minimize(self.loss)
