...
class matlqlNetwork:
    def __init__(
        ...
        self.memory = np.zeros((self.memory_size, n_features * 2 + 8))
        self.memory2 = np.zeros((self.memory_size, n_features * 2 + 8))
    ...
    def _build_net(self):
        self.s = tf.placeholder(tf.float32, [None, self.n_features+3], name='matlqls1')  
        self.q_target = tf.placeholder(tf.float32, [None, self.n_actions], name='matlqlQ_target_1')  
        
        with tf.variable_scope('matlqlValue', reuse = tf.AUTO_REUSE):
            self.name_scope = tf.get_variable_scope().name
            with tf.variable_scope('matlqleval_net_1', reuse = tf.AUTO_REUSE):
                c_names, n_l1, w_initializer, b_initializer = \
                    ['matlqleval_net_params', tf.GraphKeys.GLOBAL_VARIABLES], 50, \
                    tf.random_normal_initializer(0., 0.3), tf.constant_initializer(0.1)  

                with tf.variable_scope('matlqll_1', reuse = tf.AUTO_REUSE):
                    w1 = tf.get_variable('matlqlw_1', [self.n_features+3, n_l1], initializer=w_initializer, collections=c_names)
                    b1 = tf.get_variable('matlqlb_1', [1, n_l1], initializer=b_initializer, collections=c_names)
                    l1 = tf.nn.relu(tf.matmul(self.s, w1) + b1)

                with tf.variable_scope('matlql_lh1', reuse = tf.AUTO_REUSE):
                ...
                with tf.variable_scope('matlqll_2', reuse = tf.AUTO_REUSE):
            ...
            with tf.variable_scope('matlqlloss', reuse = tf.AUTO_REUSE):
                self.loss = tf.reduce_mean(tf.squared_difference(self.q_target, self.q_eval))
            with tf.variable_scope('matlqltrain', reuse = tf.AUTO_REUSE):
                self._train_op = tf.train.RMSPropOptimizer(self.lr).minimize(self.loss)

            self.s_ = tf.placeholder(tf.float32, [None, self.n_features+3], name='matlqls1_')    
            
            
            with tf.variable_scope('matlqltarget_net_1', reuse = tf.AUTO_REUSE):
    ...
    def _build_net2(self):
        ...
        with tf.variable_scope('matlqlAdvisor', reuse = tf.AUTO_REUSE):
            self.name_scope2 = tf.get_variable_scope().name
            with tf.variable_scope('matlqladvisor_net_1', reuse = tf.AUTO_REUSE):
                c_names, n_l1, w_initializer, b_initializer = \
                    ['matlqladvisor_net_params', tf.GraphKeys.GLOBAL_VARIABLES], 50, \
                    tf.random_normal_initializer(0., 0.3), tf.constant_initializer(0.1)  

                with tf.variable_scope('matlqll', reuse = tf.AUTO_REUSE):
                    w1 = tf.get_variable('matlqlw', [self.n_features+3, n_l1], initializer=w_initializer, collections=c_names)
                    b1 = tf.get_variable('matlqlb', [1, n_l1], initializer=b_initializer, collections=c_names)
                    l1 = tf.nn.relu(tf.matmul(self.s, w1) + b1)

                with tf.variable_scope('matlqllh1', reuse = tf.AUTO_REUSE):
                    wh1 = tf.get_variable('wh1', [n_l1, n_l1], initializer=w_initializer, collections=c_names)
                    bh1 = tf.get_variable('bh1', [1, n_l1], initializer=b_initializer, collections=c_names)
                    lh1 = tf.nn.relu(tf.matmul(l1, wh1) + bh1)
                
                with tf.variable_scope('matlqll2', reuse = tf.AUTO_REUSE):
            ...
            with tf.variable_scope('matlqlloss2', reuse = tf.AUTO_REUSE):
                self.loss2 = tf.reduce_mean(tf.squared_difference(self.q_target2, self.q_advisor))
            with tf.variable_scope('matlqltrain2', reuse = tf.AUTO_REUSE):
    ...
    def learn(self):
        ...
        q_advisornext, q_advisor = self.sess.run(
            ...
            feed_dict={
                self.s_: batch_memory[:, -(self.n_features+3):],  
                self.s: batch_memory2[:, :(self.n_features+3)],  
            })
        ...
        batch_index = np.arange(self.batch_size, dtype=np.int32)
        reward = batch_memory2[:, self.n_features+3]
        e = batch_memory2[:, self.n_features+4].astype(int)
        ...
        _, self.cost2 = self.sess.run([self._train_op2, self.loss2],
                                     feed_dict={self.s: batch_memory2[:, :(self.n_features+3)],
