...
from RL_brain_twolevelqlEM import tlqlNetworkEM
...
class twolevelqlAgentEM_differentquality(BaseAgent):
    def __init__(self, nf, sess, *args, **kwargs):
        super(twolevelqlAgentEM_differentquality, self).__init__(*args, **kwargs)
        self._recently_visited_positions = []
        self._recently_visited_length = 6
        self._prev_direction = None
        self.RL = tlqlNetworkEM(sess, 6, nf, 4,
                      learning_rate=0.01,
                      reward_decay=0.9,
                      e_greedy=0.9,
    ...
    def act(self, obs, action_space):        
        ...
        if self.execution == True: 
            ...
            if (np.random.uniform() <= self.eps):
                ...
                s =  self.RL.choose_advisor(obs2, list_actions, list_frequency, self.execution) 
            else: 
                s = 4
        ...
        list_actions.append(self.act4(obs,action_space))
        list_actions.append(self.act5(obs,action_space))
        list_actions.append(self.RL.choose_action(obs2, self.execution))
    ...
    def store(self, obs, act, reward, obs_):
        obs = featurize(obs)
        obs_ = featurize(obs_)
        self.RL.store_transition(obs, act, reward, obs_, self.advisor)
