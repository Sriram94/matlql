...
def main():
    ...
    sess = tf.Session()
    agent_list = [
        agents.twolevelqlAgentJA_differentquality(201, sess),
    ...
    with open('tlqlvsdqnJA.csv', 'w+') as myfile:
        myfile.write('{0},{1},{2}\n'.format("Episode", "Reward1(TLQL)","Reward2(DQN)"))

    with open('expertfrequencytlqlJA.csv', 'w+') as myfile:
    ...
    for i_episode in range(50000):
        ...
        with open('tlqlvsdqnJA.csv', 'a') as myfile:
            myfile.write('{0},{1},{2}\n'.format(i_episode, cumulative_rewards[0], cumulative_rewards[1]))
        with open('expertfrequencytlqlJA.csv', 'a') as myfile:
