...
def run_pursuit():
    ...
    step = 0
    with open('pettingzoosislpursuitmatlqllearningadvisor.csv', 'w+') as myfile:
        myfile.write('{0},{1}\n'.format("Episode", "sumofrewards(MATLQL)")) 

    with open('pettingzoosislpursuitmatlqlevaderslearningadvisor.csv', 'w+') as myfile:
        myfile.write('{0},{1}\n'.format("Episode", "evaders(MATLQL)")) 

 
    with open('pettingzoosislpursuitmatlqlfrequencylearningadvisor.csv', 'w+') as myfile:
    ...
    while num_episode < 2000:
        ...
        with open('pettingzoosislpursuitmatlqllearningadvisor.csv', 'a') as myfile:
            accumulated_reward = accumulated_reward/len(env.agents)
            myfile.write('{0},{1}\n'.format(num_episode, accumulated_reward))

        
        with open('pettingzoosislpursuitmatlqlevaderslearningadvisor.csv', 'a') as myfile:
