...
class CHATAgent_insufficient_similarquality(BaseAgent):
    def __init__(self, nf, sess, *args, **kwargs):
        super(CHATAgent_insufficient_similarquality, self).__init__(*args, **kwargs)
    ...
    def act(self, obs, action_space):
        ...
        a1 = self.act2(obs,action_space)
        a2 = self.act3(obs,action_space)
        a3 = self.act4(obs,action_space)
        a4 = self.act5(obs,action_space)
        a5 = self.RL.choose_action(obs_new, self.execution)
        ...
        advisor_action = random.choices(new_action_list, weights = list_action) # Choose an action based on the weight probability
        self.advisor_action = advisor_action[0]



        if a5 == 6:  #the choice is the advisor action 
            conf = self.RL.get_confidence(obs_new, self.advisor_action)
            if conf < 0.6:
                action = 0 # If confidence is less than the threshold, chat returns a placeholder action

            else:
                action = self.advisor_action
                if self.advisor_action == a1:
                    s = 0
                    self.frequency[s] = self.frequency[s] + 1

                if self.advisor_action == a2:
                    s = 1
                    self.frequency[s] = self.frequency[s] + 1
                if self.advisor_action == a3:
                    s = 2
                    self.frequency[s] = self.frequency[s] + 1
                if self.advisor_action == a4:
                    s = 3
                    self.frequency[s] = self.frequency[s] + 1

                if self.advisor_action == a5:
                    s = 4
                    self.frequency[s] = self.frequency[s] + 1
    ...
    def store(self, obs, act, reward, obs_, act_):
        obs = featurize(obs)
        obs_ = featurize(obs_)
        self.RL.store_transition(obs, act, reward, obs_, act_, self.advisor_action)
    ...
    def save_confidence_model(self, s):
        self.RL.save_confidence_model(s)

    def restore_confidence_model(self, s):
        self.RL.restore_confidence_model(s) 
