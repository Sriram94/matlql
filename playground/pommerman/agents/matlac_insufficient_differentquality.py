...
class matwolevelacAgent_insufficient_differentquality(BaseAgent):
    def __init__(self, n_f, sess, *args, **kwargs):
        super(matwolevelacAgent_insufficient_differentquality, self).__init__(*args, **kwargs)
        self._recently_visited_positions = []
        self._recently_visited_length = 6
        self._prev_direction = None

        self.actor = Actor(sess, n_features=n_f, n_actions=6, lr=0.000001)
        self.actor2 = Actor2(sess, n_features=n_f, n_advisors=4, lr=0.000001)
        self.critic = Critic(sess, n_features=n_f, n_actions=6, lr=0.001)     
        self.critic2 = Critic2(sess, n_features=n_f, n_advisors=4, lr=0.001)     
    ...
    def act(self, obs, action_space):
        ...
        if (np.random.uniform() <= self.eps):
            list_actions = []
            list_actions.append(self.act2(obs,action_space))
            list_actions.append(self.act3(obs,action_space))
            list_actions.append(self.act4(obs,action_space))
            list_actions.append(self.act5(obs,action_space))
            s = self.actor2.choose_advisor(obs2)
            if s == 0:
                a = self.act2(obs,action_space)
            ...
            self.advisor = 4
            a = self.actor.choose_action(obs2)
    ...
    def return_frequency(self):
        ...
        for i in range(len(self.frequency)):
            sum1 = sum1 + self.frequency[i]

        if sum1 == 0:
            return self.frequency
        else:
            self.frequency = [x / sum1 for x in self.frequency]
            self.frequency = [round(x*100,1) for x in self.frequency]
            return self.frequency


    
    def learn(self, obs, act, actopp, reward, obs_, actopp_, done):
        if done: 
            self.counter = self.counter + 1
            if self.counter < 50000:
                self.eps = self.linear_decay(self.counter, [0, int(50000 * 0.99), 50000], [1, 0.2, 0])
            else:
                self.eps = 0
        
        obs2 = featurize(obs)
        obs2_ = featurize(obs_)
        target_value, q_values = self.critic.learn(obs2, act, actopp, reward, obs2_, actopp_)
        self.actor.learn(obs2, act, target_value, q_values)
        if self.advisor != 4:
            target_value, q_values = self.critic2.learn(obs2, self.advisor, actopp, reward, obs2_, actopp_)
            self.actor2.learn(obs2, self.advisor, target_value, q_values)



    def actor_save_model(self, s):
        self.actor.save_model(s)


    def actor_restore_model(self, s):
        self.actor.restore_model(s)


    def actor2_save_model(self, s):
        self.actor2.save_model(s)


    def actor2_restore_model(self, s):
        self.actor2.restore_model(s)


    def critic_save_model(self, s):
        self.critic.save_model(s)


    def critic_restore_model(self, s):
        self.critic.restore_model(s)


    def critic2_save_model(self, s):
        self.critic2.save_model(s)


    def critic2_restore_model(self, s):
        self.critic2.restore_model(s)
    ...
    def act3(self, obs, action_space):
        def convert_bombs(bomb_map):
            '''Flatten outs the bomb array'''
            ret = []
            locations = np.where(bomb_map > 0)
    ...
    def act4(self, obs, action_space):
        def convert_bombs(bomb_map):
            '''Flatten outs the bomb array'''
            ret = []
            locations = np.where(bomb_map > 0)
    ...
    def act5(self, obs, action_space = 6):
    ...
    def _maybe_bomb(self, ammo, blast_strength, items, dist, my_position):
