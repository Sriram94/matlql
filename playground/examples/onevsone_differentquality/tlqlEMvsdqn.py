...
def main():
    ...
    sess = tf.Session()
    agent_list = [
        agents.twolevelqlAgentEM_differentquality(201, sess),
    ...
    with open('tlqlEMvsdqn.csv', 'w+') as myfile:
        myfile.write('{0},{1},{2}\n'.format("Episode", "Reward1(TLQLEM)","Reward2(DQN)"))

    with open('expertfrequencytlqlEM.csv', 'w+') as myfile:
    ...
    for i_episode in range(50000):
        ...
        with open('tlqlEMvsdqn.csv', 'a') as myfile:
            myfile.write('{0},{1},{2}\n'.format(i_episode, cumulative_rewards[0], cumulative_rewards[1]))
        with open('expertfrequencytlqlEM.csv', 'a') as myfile:
