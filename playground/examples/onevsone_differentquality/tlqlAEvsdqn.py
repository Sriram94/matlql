...
def main():
    ...
    sess = tf.Session()
    agent_list = [
        agents.twolevelqlAgentAE_differentquality(201, sess),
    ...
    with open('tlqlAEvsdqn.csv', 'w+') as myfile:
        myfile.write('{0},{1},{2}\n'.format("Episode", "Reward1(TLQL)","Reward2(DQN)"))

    with open('expertfrequencytlqlAE.csv', 'w+') as myfile:
    ...
    for i_episode in range(50000):
        ...
        with open('tlqlAEvsdqn.csv', 'a') as myfile:
            myfile.write('{0},{1},{2}\n'.format(i_episode, cumulative_rewards[0], cumulative_rewards[1]))
        with open('expertfrequencytlqlAE.csv', 'a') as myfile:
