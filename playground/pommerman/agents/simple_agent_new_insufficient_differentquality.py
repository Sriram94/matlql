...
class SimpleAgentNew_insufficient_differentquality(BaseAgent):
    def __init__(self, *args, **kwargs):
        super(SimpleAgentNew_insufficient_differentquality, self).__init__(*args, **kwargs)
    ...
    def act(self,obs,action_space): 
        num1 = random.randint(1,4)    #Equally samples between the 4 advisors and returns an action
        action = 0
        if num1 == 1:
            action = self.act2(obs, action_space)
        elif num1 == 2: 
            action = self.act3(obs, action_space)
        elif num1 == 3: 
            action = self.act4(obs, action_space)
        elif num1 == 4: 
            action = self.act5(obs, action_space)
