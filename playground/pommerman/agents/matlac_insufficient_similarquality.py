...
class matwolevelacAgent_insufficient_similarquality(BaseAgent):
    def __init__(self, n_f, sess, *args, **kwargs):
        super(matwolevelacAgent_insufficient_similarquality, self).__init__(*args, **kwargs)
    ...
    def act5(self, obs, action_space):
        def convert_bombs(bomb_map):
            '''Flatten outs the bomb array'''
            ret = []
            locations = np.where(bomb_map > 0)
