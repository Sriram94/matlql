...
class DeepQNetwork:
    ...
    def _build_net(self):
        ...
        with tf.variable_scope(self.name + 'DQNValue', reuse = tf.AUTO_REUSE):
            self.name_scope = tf.get_variable_scope().name
            with tf.variable_scope(self.name + 'eval_net_1', reuse = tf.AUTO_REUSE):
                c_names, n_l1, w_initializer, b_initializer = \
                    ['eval_net_params', tf.GraphKeys.GLOBAL_VARIABLES], 50, \
                    tf.random_normal_initializer(0., 0.3), tf.constant_initializer(0.1)  

                with tf.variable_scope(self.name + 'l_1', reuse = tf.AUTO_REUSE): 
                ...
                with tf.variable_scope(self.name +'l_h1', reuse = tf.AUTO_REUSE): 
                ...
                with tf.variable_scope(self.name +'l_2', reuse = tf.AUTO_REUSE): 
            ...
            with tf.variable_scope(self.name +'loss', reuse = tf.AUTO_REUSE):
                self.loss = tf.reduce_mean(tf.squared_difference(self.q_target, self.q_eval))
            with tf.variable_scope(self.name +'train', reuse = tf.AUTO_REUSE):
                self._train_op = tf.train.RMSPropOptimizer(self.lr).minimize(self.loss)

            # ------------------ build target_net ------------------
            self.s_ = tf.placeholder(tf.float32, [None, self.n_features], name='s_1_')    
            with tf.variable_scope(self.name +'target_net_1', reuse = tf.AUTO_REUSE):
